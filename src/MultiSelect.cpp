...
MultiSelect::MultiSelect(SDL_Renderer* _renderer, TTF_Font* _font, 
    ...
    : Text(_renderer, _font, _main, _x, _y, 32, FOREGROUND), font(_font) {
    Text::setBorderColor(HIGHLIGHT);
    Text::setButton([this](){
        toggled = !toggled;
        toggle();
    });
    ...
    for(MultiItem& i : _items) {
        ...
        Text* t = new Text(renderer, font, i.s.c_str(), lastRect->x, lastRect->y + lastRect->h, lastRect->w, lastRect->h, 32, FOREGROUND);
       
        lastRect = t->getRect();
       
        t->setBorderColor(HIGHLIGHT);
        t->setButton([this, i](){
            SDL_Surface* surf = TTF_RenderText_Blended(font, i.s.c_str(), 0, FOREGROUND);
